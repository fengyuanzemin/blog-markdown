...
babel有很多工具，babel-cli、babel-loader、babel API 等。
...
基于环境来转换。不包含 stage-X 的代码（babel 6）
...
使用它会根据 preset-env的 targets 配置 和 项目里面的代码 加入相应的 polyfill。不多余。而且会检测代码里面已经有 @babel/polyfill 时会提醒你代码冗余。
...
在低版本浏览器下就会出现各种问题.这个时候就需要 polyfill 了。
